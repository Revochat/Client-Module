...
try {
    ...
    client.on(EventList.User.GetFriendsReceivedList, (friends) => {
        if(friends.error) return console.log(friends.error)
        console.log("You have " + friends.length + " pending friend requests")
        console.log(friends)
    })
